...
---@return table[]
local function get_matched_items(items, pat, bang, opts)
...
local function cmd_cb(is_qf, pat, bang)
  ...
  set_list({}, ' ', {
    title = title,
    items = get_matched_items(get_list(), pat, bang),
  })
end
...
local function cmd_preview(opts, ns, preview_buf)
  ...
  local qfwinid = get_list({ winid = true }).winid
  local max_matches = vim.api.nvim_win_get_height(qfwinid)
  local lines = {}
  local items = get_matched_items(get_list(), pat, bang, { max_matches = max_matches })
  ...
  if not bang then
    for lnum = 0, vim.api.nvim_buf_line_count(qfbufnr) - 1, 1 do
      ...
      if line then
        local match_start, match_end = vim.regex(pat):match_str(line)
