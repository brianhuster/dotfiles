...
local function cmd_preview(opts, ns, preview_buf)
  ...
  local get_list = opts.name == 'Cfilter' and vim.fn.getqflist
    or function(...)
      return vim.fn.getloclist(0, ...)
    end
  ...
  local qfbufnr = qf_info.qfbufnr ---@type integer
  local qfwinid = qf_info.winid ---@type integer
  local items = qf_info.items ---@type table[]
  ...
  for _, item in ipairs(items) do
    local lnum = item.nvim_cfilter.lnum ---@type integer
